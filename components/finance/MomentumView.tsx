...
import { fetchLatestFundPrice, fundEvaluation, money, quoteSymbolForFund } from "./financeUtils";
...
function readShares() {
  ...
  try { return JSON.parse(window.localStorage.getItem(ACTUAL_SHARES_STORAGE_KEY) ?? "{}") as Record<string, number>; }
  catch { return {}; }
}

function AssetCompositionPie({ rows, total, selectedId, onSelect, onRefresh, refreshDisabled }: {
  ...
  return (
    <div className="flat-panel composition-panel">
      <div className="flat-panel-head compact-head"><div className="panel-title">構成銘柄</div><button className="btn" type="button" disabled={refreshDisabled} onClick={onRefresh}>更新</button></div>
      <div className="composition-body">
        {positiveRows.length ? <svg className="composition-pie" viewBox="0 0 100 100" role="img" aria-label="構成銘柄">
        ...
        </svg> : <div className="empty-state">評価額はまだありません。</div>}
        <div className="composition-legend">{rows.map((row, index) => <button key={row.id} type="button" className={`composition-legend-row ${row.id === selectedId ? "active" : ""}`} onClick={() => onSelect(row.id)}><span className={`legend-dot slice-${index % 8}`} /><span>{row.name}</span><b>{money(row.value)}</b></button>)}</div>
...
export function MomentumView({ title, state, selectedFund, selectedFundId, selectedTickerId, setSelectedFundId, setSelectedTickerId, updateFund, addFund, deleteFund }: {
  title?: string;
  state: FinanceState;
  ...
  async function loadSheet() {
    setLoading(true);
    setMessage("");
    try { setSheet(await fetchGooglePortfolio()); }
    catch (error) { setMessage(`スプレッドシートを取得できませんでした: ${error instanceof Error ? error.message : String(error)}`); }
    finally { setLoading(false); }
  }

  useEffect(() => { if (!isFund) void loadSheet(); }, [isFund]);

  const compositionRows = useMemo(() => {
    const shares = readShares();
    return (sheet?.rows ?? []).map((row) => ({ id: row.ticker, name: row.ticker, value: row.daily * (shares[row.ticker] ?? row.shares ?? 1) }));
  }, [sheet]);
  const compositionTotal = compositionRows.reduce((sum, row) => sum + row.value, 0);

  if (!isFund) return <section className="stack asset-product-view">
    {message && <div className="notice">{message}</div>}
    <div className="sheet-source-note">{sheet ? `${sheet.source === "cache" ? "前回取得分" : "Google スプレッドシート"}・10銘柄` : "ポートフォリオを読み込み中"}</div>
    <AssetCompositionPie rows={compositionRows} total={compositionTotal} selectedId={selectedTickerId} onSelect={setSelectedTickerId} onRefresh={() => void loadSheet()} refreshDisabled={loading} />
  </section>;
  ...
  return <section className="stack asset-product-view">
    <AssetCompositionPie rows={state.funds.map((row) => ({ id: row.id, name: row.name || "未設定", value: fundEvaluation(row) }))} total={fundTotal} selectedId={selectedFundId} onSelect={setSelectedFundId} onRefresh={() => void refreshFunds()} refreshDisabled={loading} />
    {selected && <div className="selected-asset-detail editable-selected-asset-detail compact-asset-detail">
      ...
      <div className="selected-asset-grid editable compact-asset-grid">
        ...
        <div><span>価格</span><b>{money(selected.price)}</b></div><div><span>評価額</span><b>{money(fundEvaluation(selected))}</b></div>
      </div>
    </div>}
